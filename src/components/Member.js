  ...
  renderPicks(weeklyPicks) {
    if (!this.state.clicked) {
      ...
      return weeks.map(week => {
        let classN = weeklyPicks[week].sweep ? 'border-top loss' : 'border-top'
        return <div className={classN}> {`${week}: ${weeklyPicks[week].winner || 'X'} over ${weeklyPicks[week].loser || 'X'}${weeklyPicks[week].sweep ? ' (S)' : ''} - ${weeklyPicks[week].points}`}</div>
    })
    } else {
